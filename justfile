...
contracts-gen-deployments:
    ...
    records = json.loads(pathlib.Path("crates/bindings/deployments.json").read_text())

    def body(environment):
        chain_ids = [entry["chainId"] for entry in records[environment]]
        lines = ["        chainIds = new uint256[](%d);" % len(chain_ids)]
        lines += ["        chainIds[%d] = %d;" % pair for pair in enumerate(chain_ids)]
        return "\n".join(lines)
    ...
    """
    source = "\n".join(line[4:] if line.startswith("    ") else line for line in source.split("\n"))
    source = source.replace("STAGING", body("staging")).replace("PRODUCTION", body("production"))

    out = pathlib.Path("contracts/script/RecordedDeployments.sol")
    out.write_text(source)
    subprocess.run(["forge", "fmt", "script/RecordedDeployments.sol"], cwd="contracts", check=True)
    print("generated", out)
...
contracts-deployments-check: contracts-gen-deployments
    git diff --exit-code contracts/script/RecordedDeployments.sol
