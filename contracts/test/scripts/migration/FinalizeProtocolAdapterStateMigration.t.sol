...
contract FinalizeProtocolAdapterStateMigrationTest is MigrationFixture {
    ...
    function test_run_reverts_for_a_proxy_that_copies_from_another_v1_protocol_adapter() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                FinalizeProtocolAdapterStateMigration.ProtocolAdapterV1Mismatch.selector, other, address(_v1)
            )
        );
        _finalizationScript.run({protocolAdapterV1: other, proxy: _proxy, isProduction: false});
    }
